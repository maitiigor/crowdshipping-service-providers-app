...
import { fetchChatDetail, sendMessage, uploadFile } from "@/store/slices/conversationSlice";
...
const VoiceMessagePlayer: React.FC<VoiceMessagePlayerProps> = ({
  ...
  return (
    <ThemedView
      className={`flex-row items-center gap-3 w-[300px] rounded-xl px-3 py-2 ${isSender ? "bg-white/20" : "bg-white"
        }`}
    >
      <TouchableOpacity
        ...
        disabled={!isLoaded}
        className={`h-10 w-10 items-center justify-center rounded-full ${isSender ? "bg-white" : "bg-primary-500"
          }`}
      >
        {!isLoaded ? (
...
export default function ChatDetailsScreen() {
  ...
  const { chatDetail, loading, sendingMessage, uploadingFile, error } = useAppSelector((state) => state.conversation);
  ...
  // Fetch chat detail when component mounts or conversationId changes
  useEffect(() => {
    if (conversationId) {
      dispatch(fetchChatDetail(conversationId));
    }
  }, [conversationId, dispatch]);

  // Create data object to match the old structure for compatibility
  ...
  const mutateAsync = async (messageData: {
    ...
    const result = await dispatch(sendMessage({ conversationId, ...messageData }));
    if (sendMessage.rejected.match(result)) {
      throw new Error(result.payload?.message || "Failed to send message");
    }
    // Refetch chat detail after sending message
    dispatch(fetchChatDetail(conversationId));
  ...
  const uploadFileAsync = async (formData: FormData) => {
    ...
    if (uploadFile.rejected.match(result)) {
      throw new Error(result.payload?.message || "Failed to upload file");
    }
    return result.payload;
  };
  ...
  const uploadAttachment = useCallback(
    async (
      ...
      try {
        ...
        const res = await uploadFileAsync(form);
              ...
              {isLoading ? (
                ...
                chatItems.map((item) => {
                  ...
                  return (
                    <ThemedView
                      key={item.key}
                      className={`w-full flex-row items-end ${isSender ? "justify-end" : "justify-start"
                        }`}
                    >
                      {!isSender && (
                      ...
                      <ThemedView
                        className={`max-w-[80%] rounded-2xl px-3 py-2 ${isSender ? "bg-primary-500" : "bg-typography-200"
                      ...
                      >
                            ...
                            {m.type === "voice" || m.type === "audio"
                              ...
                              : m.attachments.map((uri, index) => {
                                ...
                                return (
                                  ...
                                  >
                                    <ThemedView
                                      className={`flex-row items-center gap-2 px-3 py-2 rounded-xl ${isSender ? "bg-white/20" : "bg-white"
                                        }`}
                                    >
                                      <Icon
                                        as={Paperclip}
                        ...
                        <ThemedView className="flex-row items-center gap-1">
                          <ThemedText
                            type="c2_caption"
                            className={`${isSender ? "text-white/70" : "text-typography-700"
                              } mt-1`}
                          >
                          ...
                          <Icon
                            as={CheckCheck}
                            className={`${isSender ? "text-white/70" : "text-green-700"
                              } mt-1`}
                          />
                        </ThemedView>
                      </ThemedView>
