...
export default function MarineTripManagementScreen() {
    ...
    const trip = marineTrip;
    const locations = marineOrderLocations;
    ...
    const pickupCoords = useMemo(() => {
        ...
        if (isAir && trip.departurePort && (trip?.departurePort[0] || trip?.departurePort[1])) {
            const lat = Number(trip.departurePort[0] ? trip.departurePort[0] : null);
            const lon = Number(trip.departurePort[1] ? trip.departurePort[1] : null);
    ...
    const dropoffCoords = useMemo(() => {
        if (!trip) return null;
        if (isAir && trip.arrivalPort && (trip.arrivalPort[0] || trip.arrivalPort[1])) {
            const lat = Number(trip.arrivalPort[0] ? trip.arrivalPort[0] : null);
            const lon = Number(trip.arrivalPort[1] ? trip.arrivalPort[1] : null);
            if (!Number.isNaN(lat) && !Number.isNaN(lon)) return [lat, lon];
        }
        return null;
    }, [trip, isAir]);
                    ...
                    >
                        ...
                        {Array.isArray(dropoffCoords) && dropoffCoords.length === 2 && (
                            <Marker
                                ...
                                description={trip?.departurePort}
                                pinColor="green"
                            />
                        )}
                        ...
                        {routeCoordinates.map((point, idx) => {
                            ...
                            return (
                                <Marker
                                    ...
                                    description={formatDate(point.createdAt)}
