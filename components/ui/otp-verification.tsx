...
export default function OTPVerification({
                ...
                {Array.from({ length: pinLength }, (_, index) => (
                    ...
                    >
                        {pinLength === 4 ? (
                            ...
                            <ThemedView className={`w-4 h-4 rounded-full ${pin[index] ? 'bg-orange-500' : 'bg-transparent'
                                }`} />
                        ) : (
                            // For OTP - show numbers
                            <ThemedText className="text-2xl font-semibold text-black">
                                {pin[index] || ''}
                            </ThemedText>
                        )}
                    </ThemedView>
    ...
    const renderKeypadButton = (number: string, letters?: string) => (
        ...
        >
            <ThemedText className="text-2xl font-semibold text-black">{number}</ThemedText>
            {letters && (
                <ThemedText className="text-xs text-gray-500 mt-1">{letters}</ThemedText>
            )}
        </TouchableOpacity>
    );
    ...
    return (
        <ThemedView className="flex-1">
            ...
            <ThemedView className="px-6 mb-8 mt-8">
                <ThemedText className="text-xl font-semibold text-black text-center mb-2">
                    {title}
                </ThemedText>
            ...
            {pinLength !== 4 && (
                <ThemedView className="px-6 mb-8">
                    <Button
                        size="xl"
            ...
            {pinLength === 4 && (
                <ThemedView className="px-6 mb-8">
                    ...
                    >
                        <ThemedText className={`text-center font-semibold ${pin.length === 4 ? 'text-white' : 'text-gray-500'
                            }`}>
                            Continue →
                        </ThemedText>
                    </TouchableOpacity>
            ...
            {/* Resend Code */}
            {showResend && (
                <ThemedView className="items-center mb-8 px-6">
                    <TouchableOpacity onPress={handleResendCode} disabled={countdown > 0}>
                        <ThemedText className={`text-base ${countdown > 0 ? 'text-gray-400' : 'text-black'}`}>
            ...
            {/* Keypad */}
            <ThemedView className="flex-1 justify-end pb-8">
                <ThemedView className="bg-gray-50 p-4 rounded-t-3xl">
                    {/* Row 1 */}
                    <ThemedView className="flex-row gap-1 justify-between mb-2">
                    ...
                    {/* Row 4 */}
                    <ThemedView className="flex-row gap-1 justify-between">
                        <ThemedView className="w-1/3 h-16" />
